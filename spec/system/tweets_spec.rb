...
RSpec.describe 'ツイート投稿', type: :system do
  ...
  context 'ツイート投稿ができるとき'do
    it 'ログインしたユーザーは新規投稿できる' do
      # ログインする
        # binding.pryで止めたとして、visitの先ってなぜわかる？
        # 仮説 ユーザーが使うであろうpathを最初はこちらが指定しているだけだから
      sign_in(@user)
      # 新規投稿ページへのボタンがあることを確認する ログインされてるかどうか判断できるから
        # クリックするところとしないところの違いはなに？
        # 仮説 クリックによってカウントが変化するものを調べるため。クリックして正常にページ移動するかどうかはテスト範囲外？？
      ...
      # トップページには先ほど投稿した内容のツイートが存在することを確認する（画像）
        # イメージがセレクターの理由は？
        # 仮説 イメージは無くても投稿できるから。
        # 無い状態も含めてexpect(page).to have_content(@tweet_image)できない？　あとで実行　参考２６行目
